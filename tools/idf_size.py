...
def load_sections(map_file):
    ...
    sym_backup = None
    for line in map_file:
        # output section header, ie '.iram1.text     0x0000000040080400    0x129a5'
...
def print_summary(memory_config, sections):
    ...
    total_iram = memory_config["iram1_0_seg"]["length"]
    total_dram = memory_config["dram0_0_seg"]["length"]
    used_data = get_size(".data")
    used_bss = get_size(".bss")
    used_dram = used_data + used_bss
    used_iram = sum( get_size(s) for s in sections.keys() if s.startswith(".iram1") ) + get_size('.text')
    flash_code = get_size(".irom0.text")
    flash_rodata = get_size(".irom0.rodata")
...
def print_detailed_sizes(sections, key, header):
    ...
    for k in (sizes.keys()):
        ...
        result[k]["iram"] = sum(t for (s,t) in v.items() if s.startswith(".iram1"))
...
def print_archive_symbols(sections, archive):
    interested_sections = [".dram0.data", ".dram0.bss", ".iram1.text", ".iram1.vectors", ".flash.text", ".flash.rodata"]
    result = {}
